...
model1 <- Recommender(real_ratings, method = "SVD", param = list(method = "pearson", nn = 4))

prd <- predict(model1, real_ratings[current_user, ], type = "ratings")
...
as(prd, 'data.frame') %>% 
  ...
  datatable(class = "nowrap hover row-border", escape = FALSE, options = list(dom = 't',scrollX = TRUE, autoWidth = TRUE)) 
#*********/////////////////*******************#
#Using different approach for test & training data
...
#recommend 4 books for user 1169

pre<-predict(model_SVD,bookrate2[1169],n=4)
as(pre, "list")
...
error
# evaluation scheme
scheme <- evaluationScheme(real_ratings[1:500,], method = "cross-validation", k = 10,train=0.7, given = -1, goodRating = 5)
print(scheme)
